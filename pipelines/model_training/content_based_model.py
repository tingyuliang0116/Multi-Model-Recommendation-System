...
class ContentBasedModelWrapper(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input=None):
        ...
        result = results_df.merge(
            self.product_stats, on='product_id', how='left')
        return result[['product_id', 'title', 'brand', 'categories', 'similarity_score']]
...
class ContentBasedRecommender:
    ...
    def train_and_register(self):
        with mlflow.start_run(run_name="content_based_recommender"):
            ...
            self.product_to_idx = {pid: idx for idx,
                                   pid in enumerate(product_texts['product_id'])}
            self.idx_to_product = {idx: pid for pid,
                                   idx in self.product_to_idx.items()}
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                result = mlflow.register_model(model_uri, "ContentBasedModel")
                client = MlflowClient()
