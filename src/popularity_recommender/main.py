...
async def startup_event():
    ...
    MODEL_LOAD_TIME.labels(app_name="popularity_recommender", model_name="").set(
        perf_counter() - model_load_start_time)
    MODEL_LOAD_STATUS.labels(
        # 1 for success
        app_name="popularity_recommender", model_name="").set(1)


@app.post("/recommend/popularity", response_model=list[ProductRecommendation])
# <--- ADD THIS DECORATOR
@measure_request_latency(app_name="popularity_recommender", endpoint_name="/recommend/popularity")
async def get_recommendations(request: RecommendationRequest):
    ...
    RECOMMENDATION_COUNT.labels(app_name="popularity_recommender",
                                # <--- ADD THIS COUNTER
                                model_type="popularity").inc()
    REQUEST_COUNT.labels(
        app_name="popularity_recommender", method="POST", endpoint="/recommend/popularity", status_code=200
    ).inc()
...
@app.get("/health")
# <--- ADD THIS DECORATOR
@measure_request_latency(app_name="popularity_recommender", endpoint_name="/health")
async def health_check():
    # Ensure your model_loaded check is still here
    REQUEST_COUNT.labels(
        app_name="popularity_recommender", method="GET", endpoint="/health", status_code=200
    ).inc()
